...
export default function InventoriPage() {
  ...
  const [filter, setFilter] = useState("all"); // all, negative, low, normal
  ...
  const filteredInventory = inventoryList.filter(item => {
    ...
    if (filter === "negative") matchesFilter = item.stock < 0;
    else if (filter === "low") matchesFilter = item.stock >= 0 && item.stock <= 10;
    else if (filter === "normal") matchesFilter = item.stock > 10;
  ...
  const stats = {
    ...
    negative: inventoryList.filter(i => i.stock < 0).length,
    low: inventoryList.filter(i => i.stock >= 0 && i.stock <= 10).length,
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredInventory.length > 0 ? (
                filteredInventory.map((item) => {
                  const status = item.stock < 0 ? 'negative' : 
                               item.stock <= 10 ? 'low' : 'normal';
                    ...
                    <tr key={item.sku}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          status === 'negative' ? 'bg-red-100 text-red-800' :
