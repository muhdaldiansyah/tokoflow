  ...
  try {
    ...
    if (mappedStatus === "completed") {
      ...
      if (paymentOrder.plan_code === PACK_CODE) {
        ...
        rpcError = error;
      } else if (paymentOrder.plan_code === BISNIS_CODE_ANNUAL) {
        const annualUntil = new Date();
        annualUntil.setFullYear(annualUntil.getFullYear() + 1);
        const { error } = await supabase
          .from("profiles")
          .update({ bisnis_until: annualUntil.toISOString() })
