...
export async function GET(request: NextRequest) {
  try {
    ...
    // awaiting_payment QR orders are hidden from every active view by default.
    // The "Menunggu bayar" tab opts in with ?awaitingPayment=true.
    ...
    let query = supabase
      .from("orders")
      .select("id, user_id, order_number, customer_id, customer_name, customer_phone, items, subtotal, discount, total, delivery_fee, delivery_zone, unique_code, transfer_amount, paid_amount, notes, source, status, payment_status, delivery_date, delivery_address, is_preorder, is_dine_in, is_langganan, is_booking, booking_time, table_number, payment_claimed_at, awaiting_payment, image_urls, referral_source, assigned_staff_id, assigned_at, created_at, updated_at")
      .eq("user_id", user.id)
      .is("deleted_at", null)
      .eq("awaiting_payment", awaitingPayment)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      delivery_date, delivery_address, delivery_fee, delivery_zone, is_preorder, is_dine_in, is_langganan, is_booking,
    ...
    const deliveryFeeAmount = Math.max(0, Number(delivery_fee) || 0);
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        delivery_zone: delivery_zone === "peninsular" || delivery_zone === "sabah_sarawak" ? delivery_zone : null,
    ...
    const { error: stockError } = await serviceClient.rpc("sync_order_stock_reservations", {
      p_user_id: user.id,
      p_order_id: data.id,
      p_items: items,
    });
    ...
    if (stockError) {
      console.error("Error reserving order stock:", stockError);
      await serviceClient.from("orders").delete().eq("id", data.id).eq("user_id", user.id);
      const isInsufficient = stockError.message?.includes("insufficient_stock");
      return NextResponse.json(
        { error: isInsufficient ? "Insufficient stock for one or more items" : "Failed to reserve stock" },
        { status: isInsufficient ? 409 : 500 }
      );
    }

    // Increment order count after stock has been reserved.
    await serviceClient.rpc("increment_orders_used", { p_user_id: user.id });

    // Financial ledger — record revenue and receivable at order creation time
    if (data.total && data.total > 0) {
      serviceClient.from("financial_ledger_entries").insert({
        user_id: user.id,
        customer_id: data.customer_id ?? null,
        order_id: data.id,
        entry_type: "order_created",
        amount_delta: data.total,
        revenue_delta: data.total,
        receivable_delta: resolvedPaidAmount >= data.total ? 0 : data.total - resolvedPaidAmount,
        cash_delta: resolvedPaidAmount,
        currency: "MYR",
        actor_type: "merchant",
        actor_id: user.id,
        source_table: "orders",
        ...
        reason: `Order ${data.order_number} created`,
      }).then(() => {});
    }
