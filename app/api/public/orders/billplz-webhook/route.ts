...
export async function POST(request: Request) {
  try {
    ...
    if (state === "paid" && paid) {
      ...
      // Fire-and-forget status log so the order history timeline shows
      // "Billplz payment confirmed" without blocking the 200 response.
      if (orderId) {
        supabase.from("order_status_logs").insert({
          ...
          changed_by_name: "Billplz",
          changed_at: paidAt ?? new Date().toISOString(),
        }).then(() => {});
      }
