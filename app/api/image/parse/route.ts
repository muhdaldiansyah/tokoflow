...
export async function POST(request: NextRequest) {
  try {
    ...
    const headerPrompt = ctx.code === "MY"
      ? `You are a Malaysian SMB order parser reading a WhatsApp chat screenshot. Extract ALL information from this image into JSON.

INSTRUCTIONS:
1. Read the chat text in the screenshot and extract line items: name, qty, price.
2. Match items to the merchant's product catalog (allow common Malay / English / Manglish shortenings, e.g. "nasi lemak biasa" = "Nasi Lemak Biasa", "teh o ais" = "Teh O Ais"). Use catalog prices when matched.
3. Unknown item WITH a price → use that price. Unknown item WITHOUT a price → price: 0.
4. Convert number words: satu/one=1, dua/two=2, tiga/three=3, empat/four=4, lima/five=5, enam/six=6, tujuh/seven=7, lapan/eight=8, sembilan/nine=9, sepuluh/ten=10.
5. If quantity is not stated: qty = 1.
6. Extract the customer name if mentioned (ignore the WhatsApp sender display name — only the actual customer).
7. Extract phone if present. Malaysian formats: 01x-xxx-xxxx, 60xxxxxxxxxx, +60xxxxxxxxxx.
8. Extract delivery/pickup date if mentioned. Today is ${today}. Convert "tomorrow"/"esok"/"lusa"/"monday"/"isnin"/etc. into YYYY-MM-DD.
9. Extract special notes (e.g. "kurang manis", "extra spicy", "deliver to address X", "pack separately").
...
13. If the image is not a chat screenshot or no order is present, return empty items.`
      : `You are an Indonesian SMB order parser reading a WhatsApp chat screenshot. Extract ALL information from this image into JSON.

INSTRUCTIONS:
1. Read the chat text in the screenshot and extract line items: name, qty, price.
2. Match items to the merchant's product catalog (allow common Bahasa Indonesia / English shortenings, e.g. "nasi goreng biasa" = "Nasi Goreng Biasa", "es teh manis" = "Es Teh Manis"). Use catalog prices when matched.
3. Unknown item WITH a price → use that price (in IDR rupiah, no fractional). Unknown item WITHOUT a price → price: 0.
4. Convert number words: satu/one=1, dua/two=2, tiga/three=3, empat/four=4, lima/five=5, enam/six=6, tujuh/seven=7, delapan/eight=8, sembilan/nine=9, sepuluh/ten=10.
5. If quantity is not stated: qty = 1.
6. Extract the customer name if mentioned (ignore the WhatsApp sender display name — only the actual customer; ignore "Kak"/"Mbak"/"Bu" honorifics).
...
9. Extract special notes (e.g. "tidak pedas", "extra pedas", "kirim ke alamat X", "pack terpisah").
