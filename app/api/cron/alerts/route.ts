  ...
  for (const profile of profiles) {
    ...
    if (profile.daily_order_capacity && profile.daily_order_capacity > 0) {
      const { count: tomorrowOrders } = await supabase
        ...
        .eq("user_id", profile.id)
        // Capacity counts confirmed orders only — awaiting-payment QR ghosts are
        // excluded (migration 109), consistent with the prep list. Product stock
        // levels remain the independent oversell guard for stock-limited items.
    ...
    // Quota nudge — only fires once the starter allowance actually runs out.
    // No "X/50 used" warning at 48 — see docs/positioning/03-features.md
    // anti-anxiety rules.
