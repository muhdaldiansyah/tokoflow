...
// GET - Get current user's profile (with lazy monthly usage-counter reset)
export async function GET(request: NextRequest) {
  try {
    ...
    // Lazy cleanup: reset monthly counters on month change.
    // `orders_used` is a starter allowance counter and must not reset monthly.
    const now = new Date();
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (body.delivery_rates !== undefined) {
      if (body.delivery_rates === null) {
        updates.delivery_rates = null;
      } else if (typeof body.delivery_rates === "object" && !Array.isArray(body.delivery_rates)) {
        const allowedRateKeys = ["peninsular", "sabah_sarawak"] as const;
        const submittedRates = body.delivery_rates as Record<string, unknown>;
        const deliveryRates: Record<(typeof allowedRateKeys)[number], number | undefined> = {
          peninsular: undefined,
          sabah_sarawak: undefined,
        };

        for (const key of allowedRateKeys) {
          const rawRate = submittedRates[key];
          if (rawRate === undefined || rawRate === null || rawRate === "") continue;

          const rate = Number(rawRate);
          if (!Number.isFinite(rate) || rate < 0) {
            return NextResponse.json({ error: "Invalid delivery rates" }, { status: 400 });
          }
          deliveryRates[key] = rate;
        }

        const cleanedRates = Object.fromEntries(
          Object.entries(deliveryRates).filter(([, rate]) => rate !== undefined),
        );
        updates.delivery_rates = Object.keys(cleanedRates).length > 0 ? cleanedRates : null;
      } else {
        return NextResponse.json({ error: "Invalid delivery rates" }, { status: 400 });
      }
    }
    ...
    if (body.delivery_enabled !== undefined) updates.delivery_enabled = !!body.delivery_enabled;
    if (body.pickup_enabled !== undefined) updates.pickup_enabled = !!body.pickup_enabled;
    ...
    // Customer-payment toggle (ADR 0001). The KEYS are written via
    // /api/payments/connect (validated against Billplz first); this flag
    // just enables/disables the in-flow payment behavior on storefront.
    if (body.billplz_payment_enabled !== undefined) updates.billplz_payment_enabled = !!body.billplz_payment_enabled;
