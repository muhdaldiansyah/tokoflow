...
import { PAYMENT_MATCH_PROMPT_BY_COUNTRY, callTwinAI } from "@/lib/ai/twin-prompts";
...
export async function POST(req: NextRequest) {
  ...
  // Resolve merchant country once for prompt + currency formatting
  ...
  const ctx = resolveCountry(
    isCountry(profileRow?.country) ? profileRow.country : "ID",
  );
...
${candidates
  .map(
    (o) =>
      `- ${o.order_number} | ${o.customer_name} | ${formatMoney(o.total_amount, ctx)}${o.customer_phone ? ` | ${o.customer_phone}` : ""}`,
  )
  .join("\n")}`;

  const result = await callTwinAI({
    system: PAYMENT_MATCH_PROMPT_BY_COUNTRY[ctx.code],
    user: userPrompt,
    temperature: 0.1,
    maxTokens: 200,
  });
