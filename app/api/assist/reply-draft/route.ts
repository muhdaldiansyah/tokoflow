...
import { REPLY_DRAFT_PROMPT_BY_COUNTRY, callTwinAI } from "@/lib/ai/twin-prompts";
...
- Lifetime spend: ${formatMoney(customerCtx.total_spent, ctx)}
${customerCtx.notes ? `- Notes: ${customerCtx.notes}` : ""}`
  ...
  const merchantVoiceText = merchant_voice_notes
    ...
    : "(default warm/informal Indonesian SMB voice)";
...
Draft ONE reply for the merchant to review.`;
  ...
  const result = await callTwinAI({
    system: REPLY_DRAFT_PROMPT_BY_COUNTRY[ctx.code],
    user: userPrompt,
