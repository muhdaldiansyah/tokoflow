...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: orders, error } = await supabase
      ...
      .select("*")
      .eq("user_id", user.id)
      // Exclude QR orders awaiting a payment receipt (migration 109).
    ...
    const [
      ...
      supabase.from("orders").select("id, order_number, customer_name, delivery_date, total")
        .eq("user_id", user.id).eq("awaiting_payment", false).lt("delivery_date", todayStartIso)
        .in("status", ["new", "menunggu", "processed", "shipped"])
        .order("delivery_date", { ascending: true }).limit(10),
      supabase.from("profiles").select("orders_used, unlimited_until").eq("id", user.id).single(),
    ]);
    ...
    return NextResponse.json({
      ...
      ordersLimit: profile?.unlimited_until && new Date(profile.unlimited_until) > new Date()
        ? -1
        : FREE_STARTER_ORDERS,
    });
  } catch (error) {
