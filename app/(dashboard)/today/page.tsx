...
export default async function TodayPage() {
  ...
  const [{ data: activeOrders }, { data: doneToday }, profile, { data: outOfStockProducts }, invoicesTodayResult, customersTodayResult] = await Promise.all([
    supabase
      .from("orders")
      .select("*")
      .eq("user_id", user.id)
      .is("deleted_at", null)
      // Hide QR orders awaiting a payment receipt (migration 109) from the
      // active lineup — they live in the "Menunggu bayar" tab until paid.
