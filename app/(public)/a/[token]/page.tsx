...
export default async function CustomerAckPage({ params }: PageProps) {
  ...
  const { data: order } = await supabase
    .from("orders")
    .select(
      "id, order_number, customer_name, items, total, status, delivery_date, delivery_address, tracking_number, courier_name, customer_ack_at, user_id, deleted_at"
    )
      ...
      <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
        ...
        {acked ? (
          ...
          <>
            ...
            <div className="mx-5 mb-4 rounded-lg border bg-muted/30 px-3.5 py-3 space-y-1">
              ...
              {order.tracking_number && (() => {
                const detected = detectCourier(order.tracking_number);
                ...
                return (
                  <div className="flex items-start justify-between gap-3 pt-1">
                    ...
                    <span className="text-xs text-foreground text-right">
                      ...
                      {courier && <span className="text-muted-foreground"> · {courier}</span>}
                    </span>
                  </div>
                );
              })()}
