...
export const metadata: Metadata = {
  ...
  alternates: { canonical: `${siteConfig.url}/store` },
  openGraph: {
    type: "website",
...
export default async function TokoPage() {
  ...
  return (
    <>
      ...
      {/* Hero — restraint over hype. One-line tagline keeps the directory
          honest: it lists what's actually live, not what we wish was live.
          City + count chips below give visitors a single tap into a
          populated city page (compounds SEO when each city gets its
          own /store/[citySlug] indexed). */}
      <section className="pt-22 pb-6 bg-gradient-to-b from-emerald-50/40 to-transparent">
          ...
          {cities.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-1.5">
              ...
              {cities.slice(0, 12).map((c) => (
                <a
                  key={c.city_slug}
                  href={`/store/${c.city_slug}`}
                  className="inline-flex items-center h-7 px-2.5 rounded-full bg-card border border-border text-xs font-medium text-foreground hover:bg-muted hover:border-warm-green/30 hover:text-warm-green transition-colors"
                >
                  {c.city}
                </a>
              ))}
      ...
      {/* Directory Grid — or empty state if nobody has opted in yet. The
          empty path makes the founder/first-mover visible (the "be first"
          framing matters more than a polished grid when N=0). */}
      <section className="pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {merchants.length === 0 ? (
            <div className="max-w-xl mx-auto text-center py-16">
              <p className="text-base text-foreground font-medium">
                Tokoflow is brand new.
              </p>
              <p className="mt-2 text-sm text-muted-foreground">
                We&apos;re opening the doors to our first home sellers. If you bake, cook, cater, or craft from home — list your shop in under five minutes.
              </p>
              <a
                href="/register"
