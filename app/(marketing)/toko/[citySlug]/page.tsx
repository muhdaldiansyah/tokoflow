...
async function getCityData(citySlug: string) {
  ...
  const cityName = profiles[0].city || citySlug;
  ...
  return { merchants, categories, cityName };
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title,
    description,
    alternates: { canonical: `${siteConfig.url}/toko/${citySlug}` },
    openGraph: {
      type: "website",
      url: `${siteConfig.url}/toko/${citySlug}`,
      title,
      description,
