...
export async function getProductMovements(
  ...
  try {
    const res = await fetch(`/api/products/${productId}/movements?limit=${limit}`);
    if (!res.ok) return [];
    return res.json();
  } catch {
    return [];
...
export async function bulkUpdateStock(
  ...
  try {
    ...
    if (!res.ok) return { updated: 0, errors: ["Failed to save changes"] };
    return res.json();
  } catch {
    return { updated: 0, errors: ["Network error. Check your connection and try again."] };
  }
}
