    ...
    <div className="mx-auto w-full max-w-[1200px]">
      ...
      <div className="flex items-center justify-between gap-2 mb-5">
        <div className="flex items-center gap-2.5 min-w-0">
          <Link
            href="/products"
            className="shrink-0 h-9 w-9 flex items-center justify-center rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
          >
      ...
      <div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_280px] lg:grid-cols-[minmax(0,1fr)_320px] xl:grid-cols-[minmax(0,780px)_360px] md:items-start xl:justify-center pb-20 md:pb-6">
      ...
      <div className="rounded-lg border bg-card px-4 py-4 space-y-4 shadow-sm">
        ...
        {/* Destructive actions (edit mode) — mobile only, sidebar handles desktop */}
        {isEdit && (
          <div className="md:hidden flex items-center justify-center gap-6 pt-4 mt-2 border-t border-border">
            <button
              type="button"
            ...
            <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                <button
                  ...
                  className={`w-full h-9 flex items-center justify-between px-3 rounded-lg border transition-colors ${
                    isAvailable
                ...
                {isEdit && initialProduct && name.trim() && (
                  <button
                    ...
                    className="w-full h-9 rounded-lg border border-warm-green/30 bg-warm-green-light text-warm-green text-xs font-medium hover:bg-warm-green-light/80 transition-colors flex items-center justify-center gap-1.5 disabled:opacity-60"
                  >
                    {isGeneratingAi ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
                    {isGeneratingAi ? (imageUrl ? "Enhancing…" : "Generating…") : (imageUrl ? "Enhance with AI" : "Generate with AI")}
                  </button>
                )}
                ...
                <div className="border-t pt-3">
                  <button
                    ...
                    className="w-full h-11 rounded-lg bg-warm-green text-white text-sm font-medium hover:bg-warm-green-hover active:bg-warm-green-hover disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                  >
                ...
                {isEdit && initialProduct && (
                  <button
                    type="button"
                    onClick={() => setShowDeleteConfirm(true)}
                    className="w-full h-9 rounded-lg text-xs font-medium text-warm-rose hover:bg-warm-rose-light transition-colors"
                  >
                    Delete product
                  </button>
                )}
