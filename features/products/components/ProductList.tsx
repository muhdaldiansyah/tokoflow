...
import { Package, Plus, Camera, CircleMinus, CircleCheck, Search, Layers } from "lucide-react";
...
export function ProductList({ initialProducts, initialSales }: ProductListProps) {
  const hasInitialProducts = initialProducts !== undefined;
  const hasInitialSales = initialSales !== undefined;
  ...
  }, [hasInitialProducts, hasInitialSales]);
  ...
  const sorted = useMemo(() => {
    ...
    if (sortKey === "name") {
      ...
      filtered.sort((a, b) => {
        const soldA = sales[a.name.toLowerCase()] || 0;
        const soldB = sales[b.name.toLowerCase()] || 0;
        return sortDir === "asc" ? soldA - soldB : soldB - soldA;
      });
    } else {
  ...
  const lowStockProducts = products.filter((p) => p.stock !== null && p.stock !== undefined && p.stock > 0 && p.stock <= 3);
      ...
      <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_280px] lg:grid-cols-[minmax(0,1fr)_320px] xl:grid-cols-[minmax(0,780px)_360px] md:items-start xl:justify-center">
        <div className="min-w-0 space-y-3">
          <div className="flex items-center justify-between min-h-9">
            ...
            <div className="h-9 w-9 rounded-lg bg-muted animate-pulse md:hidden" />
          </div>
        ...
        <div className="hidden md:block">
          <div className="rounded-xl border bg-card shadow-sm h-40 animate-pulse" />
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-2">
          {products.length > 0 && (
            <Link
              ...
              className="h-9 px-3 flex items-center gap-1.5 rounded-lg text-xs font-medium border border-border bg-card hover:bg-muted transition-colors text-foreground relative"
            >
              <Layers className="w-3.5 h-3.5" />
              Inventory
              {(outOfStockCount > 0 || lowStockProducts.length > 0) && (
                <span className="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-warm-amber" />
              )}
            </Link>
          )}
          <Link
        ...
        <div className="sticky top-4 space-y-3">
          <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b bg-muted/30">
              <p className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Inventory</p>
            </div>
                ...
                <div className="border-t pt-3">
                  ...
                  <div className="space-y-1">
                    {lowStockProducts.slice(0, 3).map((p) => (
                      <Link key={p.id} href={`/products/${p.id}/edit`} className="flex items-center justify-between text-xs hover:text-foreground transition-colors">
                        <span className="text-muted-foreground truncate">{p.name}</span>
                        <span className="text-warm-amber shrink-0 ml-2">{p.stock} left</span>
                      </Link>
                    ))}
                  </div>
                </div>
