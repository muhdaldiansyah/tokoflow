...
export async function getOrders(options?: {
  ...
  try {
    ...
    if (options?.awaitingPayment) params.set("awaitingPayment", "true");
...
export async function updateOrderStatus(
  id: string,
  status: OrderStatus,
  // Optional shipment metadata captured at the same tap as "Mark as
  // Shipped" — keeps the round-trip to one PATCH instead of forcing a
  // separate edit flow.
  shipmentMeta?: { tracking_number?: string | null; courier_name?: string | null },
): Promise<Order | null> {
  try {
    const res = await fetch(`/api/orders/${id}/status`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ status, ...(shipmentMeta || {}) }),
    });
    if (!res.ok) return null;
    return res.json();
  } catch {
