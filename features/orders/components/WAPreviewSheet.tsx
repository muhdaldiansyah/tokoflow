...
interface WAPreviewSheetProps {
  ...
  onSent?: () => void;
  // Optional sub-header rendered under the contact name. Use it to signal
  // post-action context (e.g. "Status updated. Notify them?") so the merchant
  // doesn't mistake this for a confirmation gate.
  subHeader?: string;
  // Optional override for the secondary action label. Defaults to "Cancel"
  // (ad-hoc share). Pass "Skip" when the merchant is being offered a
  // follow-up notification — "Cancel" implies abort which misreads this sheet.
