...
import { Bell, X, Send, Search, CalendarDays, ChevronLeft, ChevronRight } from "lucide-react";
...
export function RemindersClient({ initialReminders }: { initialReminders: ReminderWithSource[] }) {
  const [reminders, setReminders] = useState<ReminderWithSource[]>(initialReminders);
  ...
  // Attach/detach outside click listener when calendar opens/closes
  const prevShowRef = useRef(false);
  if (showDateCalendar !== prevShowRef.current) {
    prevShowRef.current = showDateCalendar;
    if (showDateCalendar) {
