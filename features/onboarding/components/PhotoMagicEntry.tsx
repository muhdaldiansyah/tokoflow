...
export function PhotoMagicEntry({ onComplete, onDismiss }: PhotoMagicEntryProps) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (
        ...
        toast.error("Photo wasn't clear enough. Try another, or set up manually below.");
        setStage("idle");
        return;
      }
  ...
  if (stage === "idle") {
    return (
      <div className="rounded-2xl border-2 border-dashed border-warm-green/30 bg-warm-green/5 p-6 mb-6">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <Sparkles className="h-4 w-4 text-warm-green" />
              <h3 className="font-semibold text-sm text-warm-green">
                From snap to sold
              </h3>
            </div>
        ...
        <button
          ...
          className="mt-4 w-full h-12 inline-flex items-center justify-center gap-2 rounded-xl bg-warm-green text-white font-medium text-sm hover:bg-warm-green/90 transition-colors"
        >
  ...
  if (stage === "uploading") {
    return (
      <div className="rounded-2xl border-2 border-warm-green/30 bg-warm-green/5 p-8 mb-6 text-center">
        <Loader2 className="h-8 w-8 animate-spin text-warm-green mx-auto mb-3" />
    ...
    return (
      <div className="rounded-2xl border bg-card p-5 mb-6 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold text-sm">Look right?</h3>
          <button
            onClick={() => {
              setStage("idle");
              setPreview(null);
        ...
        <button
          ...
          disabled={stage === "saving"}
          className="w-full h-12 inline-flex items-center justify-center gap-2 rounded-xl bg-warm-green text-white font-medium text-sm hover:bg-warm-green/90 disabled:opacity-50 transition-colors"
        >
          {stage === "saving" ? (
            <>
