...
export function CustomerDetailClient({
  ...
  async function handleRepeatOrder() {
    try {
      const res = await fetch(`/api/customers/${customer.id}/reorder`);
      if (!res.ok) {
    ...
    <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_280px] lg:grid-cols-[minmax(0,1fr)_320px] xl:grid-cols-[minmax(0,780px)_360px] md:items-start xl:justify-center">
      ...
      <div className="flex items-center justify-between gap-2 min-h-9">
        <div className="flex items-center gap-2.5 min-w-0">
          <Link href="/customers" className="shrink-0 h-9 w-9 flex items-center justify-center rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1.5 flex-nowrap shrink-0">
                          ...
                          {order.source === "whatsapp" && (
                            <span className="inline-flex h-5 px-1.5 text-[10px] font-medium rounded-full border items-center bg-green-50 text-green-700 border-green-200">WhatsApp</span>
                          )}
            ...
            <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
              ...
              <div className="p-4 space-y-1.5">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-foreground tabular-nums">{orders.length}</span>
                </div>
                <div className="flex justify-between text-sm">
          ...
          <div className="bg-background rounded-t-2xl lg:rounded-2xl w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 pb-6 space-y-4">
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 h-11 rounded-lg bg-card border border-border shadow-sm text-xs font-medium text-foreground hover:bg-muted transition-colors">
                  Cancel
                </button>
                <button onClick={handleDelete} disabled={isDeleting} className="flex-1 h-11 rounded-lg bg-red-600 text-white text-xs font-medium hover:bg-red-700 disabled:opacity-50 transition-colors">
          ...
          <div className="bg-background rounded-t-2xl lg:rounded-2xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 pb-6 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setIsEditing(false)} className="h-8 w-8 flex items-center justify-center rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="flex gap-2">
                <button onClick={() => setIsEditing(false)} className="flex-1 h-11 rounded-lg bg-card border border-border shadow-sm text-xs font-medium text-foreground hover:bg-muted transition-colors">
                  Cancel
                </button>
                <button onClick={handleSave} disabled={isSaving} className="flex-1 h-11 rounded-lg bg-warm-green text-white text-xs font-medium hover:bg-warm-green-hover active:bg-warm-green-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                  {isSaving ? "Saving..." : "Save"}
                </button>
              </div>
              <button onClick={() => { setIsEditing(false); setShowDeleteConfirm(true); }} className="w-full h-10 flex items-center justify-center gap-1.5 text-xs font-medium text-red-600 hover:bg-red-50 rounded-lg transition-colors">
