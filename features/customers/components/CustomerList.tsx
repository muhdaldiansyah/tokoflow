...
interface CustomerListProps {
  ...
  initialPiutang?: PiutangSummary;
}

export function CustomerList({ initialCustomers, initialPiutang }: CustomerListProps) {
  const hasInitialCustomers = initialCustomers !== undefined;
  const hasInitialPiutang = initialPiutang !== undefined;

  const [customers, setCustomers] = useState<Customer[]>(initialCustomers ?? []);
  const [piutang, setPiutang] = useState<PiutangSummary | null>(initialPiutang ?? null);
  const [search, setSearch] = useState("");
  const [debouncedSearch, setDebouncedSearch] = useState("");
  const [activeTab, setActiveTab] = useState<Tab>("semua");
  const [isLoading, setIsLoading] = useState(!hasInitialCustomers);
  const skippedInitialCustomersLoad = useRef(false);
  ...
  useEffect(() => {
    ...
    if (!debouncedSearch && hasInitialCustomers && !skippedInitialCustomersLoad.current) {
      skippedInitialCustomersLoad.current = true;
      return;
    }

    getCustomers(debouncedSearch || undefined).then((data) => {
      if (!isMounted) return;
      setCustomers(data);
      setIsLoading(false);
    });
  ...
  }, [debouncedSearch, hasInitialCustomers]);

  useEffect(() => {
    if (hasInitialPiutang) return;

    let isMounted = true;
    getPiutangSummary().then((data) => {
      if (!isMounted) return;
      setPiutang(data);
    });
  ...
  const unpaidCount = piutang?.customerCount ?? 0;
  const totalDebt = piutang?.totalDebt ?? 0;
          ...
          <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
            ...
            <div className="p-4 space-y-1.5">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="tabular-nums text-foreground">{customers.length}</span>
              </div>
              {totalRevenue > 0 && (
                <div className="flex items-center justify-between text-sm">
              ...
              {totalDebt > 0 && (
                <div className="border-t pt-3 mt-1.5 space-y-1.5">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="tabular-nums text-foreground">{unpaidCount}</span>
                  </div>
                </div>
              )}
            </div>
