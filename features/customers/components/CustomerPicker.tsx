    ...
    <div ref={wrapperRef} className="grid grid-cols-1 sm:grid-cols-2 gap-2">
      <div className="relative">
        <div className="rounded-lg border bg-muted/50 transition-colors focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary/30 focus-within:bg-background">
          <label className="block px-3 pt-1.5 text-[10px] font-medium text-muted-foreground">Nama pelanggan</label>
          <input
            type="text"
            value={customerName}
      ...
      <div className="rounded-lg border bg-muted/50 transition-colors focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary/30 focus-within:bg-background">
        <label className="block px-3 pt-1.5 text-[10px] font-medium text-muted-foreground">Nomor WhatsApp</label>
        <div className="flex items-center px-3 pb-2 pt-0 gap-1.5">
          ...
          <input
            type="tel"
            value={(() => { const d = customerPhone.replace(/\D/g, ""); return d.startsWith("62") ? d.slice(2) : d.startsWith("0") ? d.slice(1) : d; })()}
            onChange={(e) => { const d = e.target.value.replace(/\D/g, ""); onPhoneChange(d.startsWith("0") ? d.slice(1) : d.startsWith("62") ? d.slice(2) : d); }}
            onFocus={() => suggestions.length > 0 && setShowSuggestions(true)}
