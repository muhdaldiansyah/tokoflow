...
export interface GroupBuySuggestion {
  category: string;
  memberCount: number;
  totalMembers: number;
  message: string;
...
interface CommunityClientProps {
  ...
  initialGroupBuy: GroupBuySuggestion[];
}

export function CommunityClient({ initialCommunities, initialAnnouncements, initialGroupBuy }: CommunityClientProps) {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      {initialGroupBuy.length > 0 && (
        <div className="space-y-2">
          <h2 className="text-sm font-semibold text-foreground flex items-center gap-1.5">
            <Package className="w-3.5 h-3.5" />
            Peluang Beli Bareng
          </h2>
          {initialGroupBuy.map(g => (
