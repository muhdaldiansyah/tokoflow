...
import { getFrequentItems, type FrequentItem } from "@/features/orders/services/order.service";
...
export function InvoiceForm({ prefill, existingInvoice }: InvoiceFormProps) {
  ...
  const [products, setProducts] = useState<FrequentItem[]>([]);
  ...
  // Load product catalog for the tap-to-add grid
  useEffect(() => {
    getFrequentItems().then((items) => {
      setProducts(items);
  ...
  function handleProductIncrement(product: FrequentItem) {
    setItems((prev) => {
      const idx = prev.findIndex((i) => i.name.toLowerCase() === product.name.toLowerCase());
      if (idx >= 0) {
        return prev.map((item, i) => i === idx ? { ...item, qty: item.qty + 1 } : item);
      }
      return [...prev.filter((i) => i.name.trim()), { name: product.name, price: product.price, qty: 1 }];
    });
  }

  function handleProductDecrement(product: FrequentItem) {
    setItems((prev) => {
      const idx = prev.findIndex((i) => i.name.toLowerCase() === product.name.toLowerCase());
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <div className="rounded-xl border bg-card shadow-sm p-4 space-y-3">
        ...
        <div className="rounded-lg border bg-card transition-colors focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary/30">
          ...
          <input
            ...
            placeholder="mis. Toko Kelontong Aisyah"
      ...
      <div className="rounded-xl border bg-card shadow-sm p-4 space-y-3">
        ...
        {/* Product grid — tap to add from catalog */}
        {productsLoading ? (
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-1.5 max-h-[200px] overflow-y-auto" style={{ maskImage: "linear-gradient(to bottom, black calc(100% - 20px), transparent)", WebkitMaskImage: "linear-gradient(to bottom, black calc(100% - 20px), transparent)" }}>
            {products.map((product) => {
              ...
              return (
                <div
                  ...
                  className="relative text-left rounded-lg border bg-muted/30 border-border hover:bg-muted/50 active:bg-muted/70 p-2 cursor-pointer transition-colors squish-press select-none"
                >
                  ...
                  <div
                    className={`flex items-center gap-2 mt-1.5 pt-1.5 border-t transition-opacity ${qty > 0 ? "border-border/50 opacity-100" : "border-transparent opacity-0 pointer-events-none"}`}
                  ...
                  >
                    ...
                    <span className="text-xs font-semibold text-foreground min-w-[16px] text-center">{qty}</span>
                    <button
                      type="button"
                      onClick={() => handleProductIncrement(product)}
                      className="h-6 w-6 rounded-md bg-warm-green text-white flex items-center justify-center hover:bg-warm-green-hover transition-colors"
                    >
              ...
              !item.name.trim() ? null : (
                <div key={index} className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 h-10 px-3 bg-card border rounded-lg shadow-sm text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary/30 transition-colors"
                  />
                  <input
                    type="number"
                    placeholder="Price"
                    ...
                    className="w-24 h-10 px-3 bg-card border rounded-lg shadow-sm text-sm text-right focus:ring-2 focus:ring-primary/20 focus:border-primary/30 transition-colors"
                  />
                  <input
                    type="number"
                    placeholder="Qty"
                    ...
                    className="w-14 h-10 px-3 bg-card border rounded-lg shadow-sm text-sm text-right focus:ring-2 focus:ring-primary/20 focus:border-primary/30 transition-colors"
                  />
                  <button
                    type="button"
                    onClick={() => removeItem(index)}
                    className="h-10 w-10 flex items-center justify-center rounded-lg border border-border hover:bg-red-50 hover:text-red-600 transition-colors shrink-0"
                  >
