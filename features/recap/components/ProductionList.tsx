...
interface ProductionListProps {
  ...
  initialProduction?: ProductionSummary | null;
...
export function ProductionList({ dateStr, selectedDate, exportTrigger, waTrigger, onExportingChange, onHasDataChange, initialProduction, initialBusinessName }: ProductionListProps) {
  const hasInitialDataRef = useRef(initialProduction !== undefined);
  const skippedInitRef = useRef(false);
  const businessNameRef = useRef(initialBusinessName || "");
  const [production, setProduction] = useState<ProductionSummary | null>(hasInitialDataRef.current ? (initialProduction ?? null) : null);
  const [isLoading, setIsLoading] = useState(!hasInitialDataRef.current);
  const [businessName, setBusinessName] = useState(initialBusinessName || "");
  const lastExportTrigger = useRef(exportTrigger);
  const lastWaTrigger = useRef(waTrigger);

  useEffect(() => {
    if (!skippedInitRef.current && hasInitialDataRef.current) {
      skippedInitRef.current = true;
      onHasDataChange(!!initialProduction && initialProduction.totalOrders > 0);
      return;
    }
    skippedInitRef.current = true;
    loadProduction();
