...
interface DailyRecapProps {
  ...
  initialProfileCreatedAt?: string;
}

export function DailyRecap({ dateStr, selectedDate, exportTrigger, onExportingChange, onHasDataChange, showAI, onCloseAI, initialBusinessName, initialProfileCreatedAt }: DailyRecapProps) {
  const businessNameRef = useRef(initialBusinessName || "");
  const profileCreatedAtRef = useRef(initialProfileCreatedAt || "");
  ...
  async function loadRecap() {
    ...
    const [data, profileResult] = await Promise.all([
      getRecapData(dateStr),
      businessNameRef.current ? Promise.resolve(null) : getProfile(),
    ]);
    setRecap(data.today);
    ...
    if (profileResult) {
      const name = profileResult.business_name || "My Store";
      businessNameRef.current = name;
      profileCreatedAtRef.current = profileResult.created_at || "";
      setBusinessName(name);
    }

    void computeMemoryCards(profileCreatedAtRef.current);
  ...
  async function computeMemoryCards(profileCreatedAt: string | undefined) {
    ...
    try {
      ...
      const { count } = await supabase
        ...
        .is("deleted_at", null)
        // Don't count QR orders awaiting a payment receipt (migration 109).
  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      {/* Usage */}
      <div className="rounded-xl border bg-card px-4 py-4 space-y-2 shadow-sm">
          <p className="text-xs font-medium text-muted-foreground">Order usage</p>
      ...
      {recap && recap.totalOrders > 0 && (
        <div className="rounded-xl border bg-card px-4 py-4 shadow-sm">
          ...
          <button
            ...
            className="w-full h-10 rounded-lg bg-warm-green text-white text-sm font-medium hover:bg-warm-green-hover transition-colors flex items-center justify-center gap-2"
          >
