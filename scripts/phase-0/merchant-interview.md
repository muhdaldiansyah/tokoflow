...
### 8. Marketplace appetite (3 min) — added 2026-05-19 for Stage 2-3 validation

After [`07-decisions.md` D-018](../../docs/positioning/07-decisions.md) (marketplace trajectory, drafted post-Tokoflow→Tokoflow rebrand), we need real-merchant signal before investing in directory polish (Stage 2) or customer accounts (Stage 3). 5 quick questions — wrap before the brand probe so the brand answer doesn't get contaminated by "the marketplace thing."

**Ask in order; don't lead.**

**Q1 — Marketplace dependency baseline:**
> *"Sekarang ni, berapa peratus revenue kamu dari Shopee / TikTok Shop / Foodpanda / Lazada vs direct customer (WhatsApp, IG, walk-in)?"*

Score: % marketplace-dependent. Used to predict willingness to switch.

**Q2 — Commission-for-discovery trade test:**
> *"Kalau ada platform charge 2-3% commission tapi (a) tool macam Tokoflow built-in (b) customer kamu boleh discover kedai kamu kat directory platform — sanggup switch dari current setup?"*

Score 1-10 willingness. ≥7 = strong marketplace pull. <4 = pure tool play is right.

**Q3 — Customer-side magic link validation (Model D test):**
> *"Kalau customer kamu boleh tap satu link untuk lihat semua past order dengan kamu — tanpa kena register, tanpa password — useful tak untuk customer kamu? (Bukan untuk customer akaun Tokoflow ye, ni link khas dari kamu ke customer)."*

Score Yes/No + qualitative reaction. Validates [[Model D]] from per-merchant magic link analysis.

**Q4 — Directory opt-in (Stage 2 test):**
> *"Kalau Tokoflow ada page macam `/store/shah-alam` yang list semua home F&B di Shah Alam — customer Google search 'kuih shah alam' boleh jumpa kamu — mahu opt-in untuk dilist?"*

Probe: *"Apa concerns? Kompetitor nampak harga kamu? Customer pilih based on price je?"*
Score Yes/No + concern themes. Validates Stage 2 directory activation appetite.

**Q5 — Trust / differentiation question:**
> *"Apa yang akan buat kamu trust Tokoflow as marketplace berbanding Shopee atau Foodpanda? Apa kena ada / kena tak ada?"*

Open-ended. Looking for defensibility signal — what's the moat beyond commission %?

**Log to interview log**: 4 new columns (added):
- `Marketplace dep %` (Q1, numeric 0-100)
- `Switch willingness 1-10` (Q2)
- `Magic link Y/N` (Q3)
- `Directory opt-in Y/N + concerns` (Q4)

**Decision thresholds** (after all 10 interviews):
- Q2 avg ≥7 → **proceed Stage 3 marketplace planning post-Phase 0**
- Q2 avg 4-6 → **soft directory only (Stage 2), defer Stage 3**
- Q2 avg <4 → **pure tool play, freeze marketplace pivot**
- Q3 ≥7/10 yes → **build Model D magic link immediately**
- Q4 ≥7/10 opt-in → **invest in directory SEO + curation**

---

### 9. Brand resonance test (2 min) — added per SYNTHESIS-2026-05-05.md §6.1
