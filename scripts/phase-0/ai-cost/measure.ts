 ...
 * (Gemini 3.5 Flash — production primary) and measures actual token usage + cost. Extrapolates to
...
const MODEL = "google/gemini-3.5-flash"; // production primary — keep in sync with lib/ai/model.ts

// OpenRouter pricing (verify at https://openrouter.ai/google/gemini-3.5-flash — refresh quarterly)
  ...
  const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        {
          role: "user",
          // Vision scenarios attach an image so --full measures real image
          // tokenization; text scenarios send a plain string.
          content: imageUrl
            ? [
...
async function measureScenario(
  ...
  // Vision/parse scenarios carry their own systemPrompt; twin/assist ones
  // resolve from the shared ./prompts/* files.
  const systemPrompt = scenario.systemPrompt ?? getSystemPrompt(scenario.type);
  const userInput = scenario.inputContext;

  if (dryRun) {
    // Image tokens (Gemini tiles the image) aren't captured by char-counting —
    // add the per-scenario estimate so the dry-run total includes vision ops.
    const inputTokens = estimateTokens(systemPrompt + userInput) + (scenario.imageTokens ?? 0);
...
function formatReport(report: AggregatedReport, dryRun: boolean): string {
  ...
  if (report.pricingTierVerdict === "PASS_AMPLE") {
    ...
    lines.push("- Or move high-frequency mechanical ops (payment_match, pattern_detection, order parse) to a cheaper lite model (gemini-2.5-flash-lite / gemini-3.1-flash-lite) and keep 3.5-flash only for reply drafts + receipt triage");
