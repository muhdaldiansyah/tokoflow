...
## D-018 · Marketplace trajectory — 4-stage roadmap, Refuse #6 refined (2026-05-19)

**Context.** Founder feedback 2026-05-19 explicitly named the gap that v1.2 left ambiguous: Tokoflow needs to be findable, not just usable. Malaysian merchants are bleeding margin to Shopee/Foodpanda/TikTok Shop (25-35% commissions) and actively seeking alternatives — the merchant-tool-only framing of v1.2 doesn't capture that demand. The /toko (now /store) directory infrastructure was already built (migration 060, DirectoryGrid, profiles.is_listed) but disabled via next.config redirect — a leftover deferral from v1.2 lock. Activating it forces a positioning question: is Tokoflow a tool, or a marketplace?

**Decision.** Both, in sequence — never simultaneously. Bible v1.3 introduces a 4-stage trajectory and refines Refuse #6 to make ownership preservation explicit on the direct-order axis while permitting platform-identity on the discovery axis.

**Four stages:**

| Stage | When | Surface | Customer identity | Commission |
|---|---|---|---|---|
| **1 — Merchant beachhead** | Now → Month 6 | Tool + activated `/store` directory + per-merchant magic link (Model D, deferred until Phase 0 validates) | None (anonymous) | 0% |
| **2 — Soft directory** | Month 6-12 (gated by Stage 1 PMF) | `/store/[city]`, `/store?category=`, opt-in listing, SEO-driven discovery | None (anonymous) | 0% |
| **3 — Light marketplace** | Year 2 (gated by Stage 2 traffic + Phase 0 marketplace appetite ≥7/10 Q2) | Platform-wide customer account (Model B), reviews, recommendations, wallet | Required | **2-3% on marketplace-driven orders only**; 0% on direct merchant link |
| **4 — Full marketplace** | Year 3-5 (gated by Stage 3 unit economics) | Cross-merchant cart, escrow, dispute resolution, logistics aggregation | Required | 2-3% across all marketplace flows |

**Refuse #6 refinement.** The original v1.2 wording — *"Klaim ownership customer (data tetap milikmu)"* — is preserved as the **direct-order tenet** and sharpened at the discovery edge:

> **Refuse #6 (refined v1.3):** Tokoflow tidak claim ownership over merchant's direct customer relationships. Where Tokoflow facilitates new customer discovery (Stage 3+ marketplace), the platform manages identity for that discovery channel only — merchant retains full transaction data, customer phone/name, and post-discovery relationship rights. **Direct merchant-link orders are 0% commission and merchant-owned. Marketplace-discovered orders are 2-3% commission with merchant-owned transaction data and platform-owned discovery surface.**

**Two-track economics (new).** This is the structural innovation that lets Tokoflow be a marketplace without becoming Shopee:
- **Direct track:** customer arrives via merchant's shared link (`/[slug]`), or WA, or IG bio → 0% commission, merchant owns customer 100%
- **Marketplace track:** customer arrives via `/store` directory + clicked merchant card → 2-3% commission, marketplace logged via `orders.referral_source` (column already exists per migration 060)

Existing `referral_source` column was deliberately added in 060 anticipating this — the foundation has always been here; v1.3 names what was implicit.

**Why this works (efficient marketplace thesis).** Shopee charges 5-15% effective take to cover ops cost of ~20-30% of GMV (paid CAC, large support team, dispute resolution at scale, fraud). Tokoflow can sustain 2-3% commission if ops cost stays at 5-10% of GMV — achievable via AI-automated support (Background Twin foundation), vertical focus (lower dispute complexity), organic CAC (each merchant brings their customers), strict KYC (low fraud). This is Toast's playbook (restaurant POS → marketplace, ~7 years), not Shopee's (burn $billions on CAC).

**What this is NOT.** This is NOT permission to skip stages. PgMall, PrestoMall, 11street MY all died trying to be horizontal marketplaces from Day 1. Stage 1 PMF (merchant tool + 50-100 paying merchants) MUST land before Stage 2 polish; Stage 2 traffic MUST validate before Stage 3 build. **No stage-jumping. No founder override of stage discipline.**

**Phase 0 validation gates (added to merchant-interview.md §8).** 5 questions tied to stage progression:
- Q1 baseline (marketplace dependency %)
- Q2 commission-for-discovery willingness (≥7/10 avg = Stage 3 green light)
- Q3 magic-link validation (Model D ≥7/10 yes = build immediately)
- Q4 directory opt-in (Stage 2 appetite)
- Q5 trust/differentiation (defensibility signal)

**What this OVERRIDES from v1.2.** Nothing structural — v1.2's Three-Tier Reality (D-013), 2-Layer Twin (D-014), lifestyle/venture acceptance (D-015), and Refuses #1-5 + #7-10 all stand. **Only Refuse #6 receives a refinement** (not a replacement). Bible v1.2 archive files (`00-manifesto.md` through `08-the-disappearing-work.md`) are left intact; v1.3 deltas land additively as new docs + this D-018 entry.

**Killcriterion update.** Add to existing kill triggers:
- **Stage 1 PMF gate fails** (< 5/10 interview resonance with Three-Tier OR Phase 0 AI cost >RM 30/merchant/month) → kill, no Stage 2 attempt
- **Stage 2 traffic gate fails** (< 100 directory visits/week sustained over 6 weeks post-launch) → freeze at Stage 1, defer Stage 3 indefinitely
- **Stage 3 unit economics fails** (marketplace commission < ops cost) → kill marketplace, retain tool-only model

**Companion doc.** Full trajectory expansion + analog mapping (Toast, Shopify, Square, Etsy, Substack) in [`09-marketplace-trajectory.md`](./09-marketplace-trajectory.md).

**Status.** Decided 2026-05-19 in founder discussion. Bible promoted to v1.3. v1.2 archive preserved.
...
- Marketplace trajectory expansion: [`09-marketplace-trajectory.md`](./09-marketplace-trajectory.md)
- All other docs reference back here untuk reasoning

---

*Versi 1.3 · 19 May 2026 · Decisions log adalah memory institutional. Tidak ada keputusan yang dibuat tanpa dicatat di sini.*
...
*Changelog 1.3 (2026-05-19):* Added D-018 (marketplace trajectory, 4-stage roadmap, Refuse #6 refined, two-track economics). Source: founder discussion 2026-05-19 + Stage 1 directory activation (commit baa347f, renamed /toko → /store in 01286cf). Companion doc `09-marketplace-trajectory.md` added.
